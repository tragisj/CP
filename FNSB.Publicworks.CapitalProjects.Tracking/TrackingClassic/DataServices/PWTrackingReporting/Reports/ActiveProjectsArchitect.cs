...
namespace FNSB.Publicworks.Projects.Reporting.Reports
{
    ...
    /// </summary>
    public partial class ActiveProjectsArchitect : GrapeCity.ActiveReports.SectionReport
    {
        public ActiveProjectsArchitect(int architectId)
        {
            //
            // Required for Windows Form Designer support
            //
            InitializeComponent();
            _selectId = architectId;
        }
        ...
        private void ActiveProjectsArchitectDataInitialize(object sender, EventArgs e)
        {
            ...
            switch (_selectId)
            {
                case 0: //all architect report
                    ...
                    _projects = from architectEngineer in _ctx.ArchitectEngineers  //query all secretaries that have active projects
                                from project in architectEngineer.Projects
                                where project.PPM_Project_Active == true && project.ppm_project_complete == false
                                orderby project.ArchitectEngineer.lastname, project.PPM_Project_Number
                    ...
                    {
                        arcengHeaderText.Text = _ctx.ArchitectEngineers.Find(_selectId).FullName;
                ...
                default:    //ONE Architect

                    _projects = from architectEngineer in _ctx.ArchitectEngineers  //active projects for the secretary selected from the sec dialog
                                from project in architectEngineer.Projects
                                where project.PPM_Project_Active == true && project.ppm_project_complete == false &&
                                project.PPA_Recordid == _selectId
                                orderby project.ArchitectEngineer.lastname, project.PPM_Project_Number
                    ...
                    if (j == 0)
                    {
                        arcengHeaderText.Value = _ctx.ArchitectEngineers.Find(_selectId).FullName;   //no active projects for this secretary display
        ...
        private void ActiveProjectsArchitectFetchData(object sender, FetchEventArgs eArgs)
        {
        ...
        private void GroupHeader1Format(object sender, EventArgs e)
        {
            ...
            var id = _projectList[_count].ArchitectEngineer.recordid;

            _groupCount = _ctx.ArchitectEngineers.Find(id).Projects.Count(a => a.PPM_Project_Active == true);

            _budgetTotal = (decimal)_ctx.ArchitectEngineers.Find(id).Projects.Where(a => a.PPM_Project_Active == true).ToList().Sum(
                    f => f.Funds.ToList().Sum(d => d.ppf_budget));

            _balanceTotal = (decimal)_ctx.ArchitectEngineers.Find(id).Projects.Where(a => a.PPM_Project_Active == true).ToList().Sum(
            ...
            groupHeader1.AddBookmark(Fields[3].Value.ToString());
        }

        private void ActiveProjectsArchitect_ReportStart(object sender, EventArgs e)
        {
