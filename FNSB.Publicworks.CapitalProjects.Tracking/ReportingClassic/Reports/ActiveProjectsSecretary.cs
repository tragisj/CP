...
namespace FNSB.Publicworks.Projects.Reporting.Reports
{
    ...
    /// </summary>
    public partial class ActiveProjectsSecretary : GrapeCity.ActiveReports.SectionReport
    {
        ...
        public ActiveProjectsSecretary(int secretaryId)
        {
            //
        ...
        private void ActiveProjectsSecretaryDataInitialize(object sender, EventArgs e)
        {
            ...
            switch (_selectId)
            {
                case 0: //all architect report
                    ...
                    _projects = from projects in _ctx.Secretaries  //query all secretaries that have active projects
                                from project in projects.Projects
                                where project.PPM_Project_Active == true && project.ppm_project_complete == false
                                orderby project.Secretary.lastname, project.PPM_Project_Number
                    ...
                    {
                        secretaryHeaderText.Text = _ctx.Secretaries.Find(_selectId).FullName;
                ...
                default:    //ONE ProjectManager
                    ...
                    _projects = from projects in _ctx.Secretaries  //active projects for the secretary selected from the sec dialog
                                from project in projects.Projects
                                where (project.PPM_Project_Active == true && project.ppm_project_complete == false) && project.PPS_Recordid == _selectId
                                orderby project.Secretary.lastname, project.PPM_Project_Number
                    ...
                    if (j == 0)
                    {
                        secretaryHeaderText.Value = _ctx.Secretaries.Find(_selectId).FullName;   //no active projects for this secretary display
        ...
        private void GroupHeader1Format(object sender, EventArgs e)
        {
            ...
            var id = _projectList[_count].Secretary.pps_recordid;

            _groupCount = _ctx.Secretaries.Find(id).Projects.Count(a => a.PPM_Project_Active == true);

            _budgetTotal = (decimal)_ctx.Secretaries.Find(id).Projects.Where(a => a.PPM_Project_Active == true).ToList().Sum(
                    f => f.Funds.ToList().Sum(d => d.ppf_budget));

            _balanceTotal = (decimal)_ctx.Secretaries.Find(id).Projects.Where(a => a.PPM_Project_Active == true).ToList().Sum(
